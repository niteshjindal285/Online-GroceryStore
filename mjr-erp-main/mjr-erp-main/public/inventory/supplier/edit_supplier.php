...
$page_title = 'Edit Supplier - MJR Group ERP';
$errors = [];
...
if (is_post()) {
    ...
    if (verify_csrf_token($csrf_token)) {
        try {
            ...
            // Check if code exists for another supplier in this company
            $check_sql = "SELECT id FROM suppliers WHERE supplier_code = ? AND id != ?";
            $check_params = [$supplier_code, $supplier_id];
            ...
            $existing = db_fetch($check_sql, $check_params);
            if ($existing) {
                $errors['supplier_code'] = 'Supplier code already exists for this company';
                throw new Exception('Validation failed.');
            }
            
            // Update supplier - only for current company if supported
            $sql = "UPDATE suppliers 
                    SET supplier_code = ?, name = ?, contact_person = ?, email = ?, phone = ?, 
                        address = ?, city = ?, country = ?, payment_terms = ?, tax_id = ?, 
                        is_active = ?, notes = ?
                    WHERE id = ?";
            $params = [
                $supplier_code,
            ...
            set_flash('Supplier updated successfully!', 'success');
            redirect('suppliers.php');
...
<div class="container">
    <div class="row mb-4">
        <div class="col">
            ...
            <p class="text-muted mb-0">Editing for <strong><?= escape_html(active_company_name()) ?></strong></p>
        </div>
        <div class="col-auto">
            ...
            <form method="POST">
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control <?= isset($errors['supplier_code']) ? 'is-invalid' : '' ?>" id="supplier_code" name="supplier_code" 
                               value="<?= escape_html(post('supplier_code') ?: $supplier['supplier_code']) ?>" required>
                    ...
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control <?= isset($errors['name']) ? 'is-invalid' : '' ?>" id="name" name="name" 
                               value="<?= escape_html(post('name') ?: $supplier['name']) ?>" required>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="contact_person" name="contact_person"
                               value="<?= escape_html($supplier['contact_person'] ?? '') ?>">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="email" class="form-label">Email</label>
                        <input type="email" class="form-control" id="email" name="email"
                               value="<?= escape_html($supplier['email'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="address" name="address" rows="2"><?= escape_html($supplier['address'] ?? '') ?></textarea>
                </div>
                ...
                <div class="row">
                    <div class="col-md-6 mb-3">
                        ...
                        <input type="text" class="form-control" id="city" name="city"
                               value="<?= escape_html($supplier['city'] ?? '') ?>">
                    </div>
                    <div class="col-md-6 mb-3">
                        <label for="country" class="form-label">Country</label>
                        <input type="text" class="form-control" id="country" name="country"
                               value="<?= escape_html($supplier['country'] ?? '') ?>">
                    </div>
                </div>
                ...
                <div class="mb-3">
                    ...
                    <textarea class="form-control" id="notes" name="notes" rows="3"><?= escape_html($supplier['notes'] ?? '') ?></textarea>
                </div>
                    ...
                    <div class="card p-3 border-secondary bg-dark text-white">
                        <div class="form-check form-switch ms-1">
                            <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1" 
                                   <?= (is_post() ? (has_post('is_active') ? 'checked' : '') : ($supplier['is_active'] ? 'checked' : '')) ?> style="width: 2.5rem; height: 1.25rem; cursor: pointer;">
                            <label class="form-check-label fw-bold ms-3 d-inline-flex align-items-center gap-2" for="is_active" style="cursor: pointer; padding-top: 2px;">
                                ...
                                <span id="supplier_active_badge" class="badge <?= (is_post() ? (has_post('is_active') ? 'bg-success' : 'bg-secondary') : ($supplier['is_active'] ? 'bg-success' : 'bg-secondary')) ?>">
                                    <i class="fas <?= (is_post() ? (has_post('is_active') ? 'fa-check-circle' : 'fa-minus-circle') : ($supplier['is_active'] ? 'fa-check-circle' : 'fa-minus-circle')) ?> me-1"></i>
                                    <?= (is_post() ? (has_post('is_active') ? 'Active' : 'Inactive') : ($supplier['is_active'] ? 'Active' : 'Inactive')) ?>
                                </span>
                            </label>
                        </div>
                    </div>
