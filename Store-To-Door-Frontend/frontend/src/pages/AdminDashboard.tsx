  ...
  useEffect(() => {
    ...
    const fetchStats = async () => {
      ...
      try {
        ...
        const orders = (Array.isArray(ordersResponse.data) ? ordersResponse.data : []) as Order[];
        const revenue = orders.reduce((sum: number, o: Order) => sum + (o.totalPrice || 0), 0);
        const recentOrdersData = orders.slice(0, 5).map((o: Order) => ({
          id: o._id || o.id || '',
          customer: o.shippingAddress?.name || 'Customer',
          items: (o.items?.length || 0) as number,
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {[
              ...
              { 
                ...
                desc: 'View financial statements and transactions.', 
                href: '/erp/finance', 
                icon: BarChart3, 
              ...
              <div 
                ...
                className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm hover:shadow-md hover:-translate-y-1 transition-all duration-300 cursor-pointer group"
              >
                <div className={`w-12 h-12 ${module.color} flex items-center justify-center rounded-xl mb-5 group-hover:scale-110 transition-transform duration-300`}>
                  <module.icon className="h-6 w-6" />
                </div>
                <h3 className="text-lg font-bold text-gray-900 mb-2 truncate">{module.title}</h3>
                <p className="text-sm text-gray-500 leading-relaxed mb-4">{module.desc}</p>
                <div className="flex items-center text-xs font-bold text-emerald-600 group-hover:gap-2 gap-1 transition-all">
